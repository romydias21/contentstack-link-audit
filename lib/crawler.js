...
const rawMaxPages = Number(process.env.MAX_PAGES);
const DEFAULT_MAX_PAGES = Number.isFinite(rawMaxPages) && rawMaxPages >= 0 ? rawMaxPages : 50000;
const rawRenderJs = process.env.RENDER_JS;
const DEFAULT_RENDER_JS = rawRenderJs ? rawRenderJs.toLowerCase() === 'true' : false;
...
async function createBrowserContext(config) {
  if (!config.renderJs) return null;
  let chromium;
  try {
    ({ chromium } = require('playwright-chromium'));
  } catch (err) {
    throw new Error('Playwright is not available. Set RENDER_JS=false to disable JS rendering.');
  }
...
async function runAudit({ startUrl, onProgress, configOverrides }) {
  ...
  config.renderJs = parseBoolean(config.renderJs, DEFAULT_RENDER_JS);
  ...
  let context;
  try {
    const browserBundle = await createBrowserContext(config);
    if (browserBundle) {
      browser = browserBundle.browser;
      context = browserBundle.context;
    }
  } catch (err) {
    if (config.renderJs) {
      throw err;
  ...
  function trackQueues() {
    tracker.update({
      pagesQueued: pendingPages.size,
      pagesDiscovered: pendingPages.size + visitedPages.size,
  ...
  function enqueuePage(url) {
    ...
    if (config.maxPages !== 0 && visitedPages.size + pendingPages.size >= config.maxPages) return;
